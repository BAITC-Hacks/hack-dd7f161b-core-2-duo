...
def quantiles(values):
    """Линейная интерполяция позиции (n−1)·p; единица — миллисекунда."""
...
def markdown(report):
    ...
    lines = [
        ...
        "Датой расчёта служит as_of_date набора, а не календарная дата запуска. Рекомендации — все показанные top-3, "
        ...
        "он сохраняется: корректная верхняя граница after — max(before, cap), при шкале ≤5. "
    ...
    lines += [
        ...
        "Ниже знаменатель — все сотрудники без рекомендаций, включая выполненные цели. "
    ...
    lines += [
        ...
        "проверок. Загрузка файлов/импорт не входят. Квантили — линейная интерполяция позиции (n−1)·p. "
    ...
    lines += [
        ...
        "Числитель здесь — нарушения, одно событие может нарушить несколько инвариантов.",
    ]
    if base["failed_cases"]:
        ...
        for failure in base["failed_cases"]:
            ...
            lines.append(
                f"| {METRIC_LABELS.get(failure['check'], failure['check'])} | {failure['employee_id'] or '—'} | {failure['event_id'] or '—'} | {details} |"
            )
